...
class BiasBalancedTopKRouter(TopKRouter):
    ...
    def __init__(
        ...
        super().__init__(hidden_dim, num_experts, top_k, z_loss_weight, temperature)
        if bias_lr < 0:
    ...
    def forward(self, x: Tensor) -> RoutingResult:
        ...
        result.aux = {"routing_bias": self.routing_bias.detach().clone()}
        if self.z_loss_weight:
            result.z_loss = self.z_loss_weight * self.z_loss(logits)
        return result

    @torch.no_grad()
    def update_bias(self, expert_load: Tensor, target_load: float | Tensor) -> None:
        load = expert_load.to(self.routing_bias.device, dtype=self.routing_bias.dtype)
        if isinstance(target_load, Tensor):
            target = target_load.to(load.device, load.dtype)
        else:
            target = torch.full_like(load, float(target_load))
        self.routing_bias.add_(self.bias_lr * torch.sign(target - load))
