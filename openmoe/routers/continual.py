...
class ExpertRoutingMemory(nn.Module):
    ...
    def update(
        ...
        flat_x = flat_x.repeat_interleave(
            indices.shape[-1],
            dim=0,
        )
        ...
        for expert_id in flat_i.unique().tolist():
            ...
            mean = F.normalize(
                flat_x[mask].mean(dim=0),
                dim=0,
            )

            self.prototypes[expert_id].mul_(
                self.momentum
            ).add_(
                (1.0 - self.momentum) * mean
            )

            self.prototypes[expert_id].copy_(
                F.normalize(
                    self.prototypes[expert_id],
                    dim=0,
                )
            )
...
class ContinualRouter(BiasBalancedTopKRouter):
    ...
    def __init__(
        ...
        self.memory = ExpertRoutingMemory(
            hidden_dim,
            num_experts,
            memory_momentum,
        )
    ...
    def observe(
        ...
        # A frozen router must not change its continual memory.
