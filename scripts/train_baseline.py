...
def observe_continual_memory(model: TinyMoETransformer, images: torch.Tensor) -> None:
    """Update non-parametric routing memory outside the autograd graph."""
    model.eval()
    with torch.no_grad():
        x = model.patch_embed(images).flatten(2).transpose(1, 2) + model.pos_embed
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a reproducible OpenMoE-Router baseline.")
    parser.add_argument("--config", default="configs/phase1.yaml")
    parser.add_argument("--router", default="continual", choices=["dense", "top1", "top2", "bias", "continual"])
    parser.add_argument("--steps", type=int, default=50)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--data", choices=["synthetic", "cifar100"], default="synthetic")
    parser.add_argument("--output", default="experiments/results/baseline.json")
    ...
    tasks = int(cfg["experiment"]["tasks"])
    classes_per_task = int(cfg["experiment"].get("classes_per_task", 5))
    num_classes = tasks * classes_per_task
    if args.data == "synthetic":
        stream = make_synthetic_stream(tasks=tasks, classes_per_task=classes_per_task, seed=args.seed)
    else:
        stream = build_split_cifar100_stream(tasks=tasks)
        num_classes = 100
    ...
    if args.router == "dense":
        ...
        model = TinyMoETransformer(
            ...
            router_factory=make_router_factory(args.router, cfg),
            depth=2,
        ).to(device)
    ...
    for task_id, loader in enumerate(stream):
        warmup = min(args.steps, 10)
        train_steps(model, loader, optimizer, device, warmup)
        if task_id == 0 and args.router != "dense" and cfg["continual"].get("freeze_experts_after_warmup", False):
            model.set_experts_trainable(False)
        remaining = max(args.steps - warmup, 0)

        def after_step(images, _output):
            if args.router == "continual":
                observe_continual_memory(model, images)

        train_steps(model, loader, optimizer, device, remaining, post_step=after_step)
        row = [evaluate(model, seen_loader, device) for seen_loader in stream[: task_id + 1]]
        accuracies.append(row)
        print(f"task={task_id} accuracies={row}")
